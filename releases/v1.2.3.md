...
* fix(ci): use job-level env for RubyGems secret gate

Secrets context cannot be used bare in step-level if conditions. Move the secret check to a job-level env var (HAS_RUBYGEMS_KEY) evaluated in an expression context where secrets are available, then reference it via env in the step condition. Ref #45.
